...
from BEV.project_mask_to_bev import project_mask_to_bev
...
def cals_BEV(onnx_path, img_rgb_nchw, side, try_cuda=False):
    # 选择 EP
    ...
    img_rgb_nchw = img_rgb_nchw.astype(in_dtype, copy=False)

    # 推理
    t0 = time.time()
    y = sess.run([out.name], {inp.name: img_rgb_nchw})[0]
    ...
    try:
        bev_output_path = 'BEV_' + side + '.png'
        bev_mask, src_pts_xy, H_final, dst_pts_bev_px, f_pass_corner_check = project_mask_to_bev(mask, bev_output_path)
        print(f"Passed 4 corners check: {f_pass_corner_check}")
        img = Image.open(bev_output_path)  # this is to verify the BEV result to confirm if it is good.
        img.show()

        if side == 'left':
            np.save("BEV/H_final_left.npy", H_final)
            np.save("BEV/bev_mask_left.npy", bev_mask)
            np.save("BEV/dst_pts_bev_px_left.npy", dst_pts_bev_px)
            np.save("BEV/mask_left.npy", mask)
        elif side == 'right':
            np.save("BEV/H_final_right.npy", H_final)
            np.save("BEV/bev_mask_right.npy", bev_mask)
            np.save("BEV/dst_pts_bev_px_right.npy", dst_pts_bev_px)
            np.save("BEV/mask_right.npy", mask)
        # Till here, calibration is done.
