...
from features.dribble_feature_simu import DribbleFeature

FP32, I32 = 4, 4  # bytes per float32 / int32
SHOOT_MODES = [
    'mid-range baseline', '3-points baseline', 'mid-range star',
    '3-points star', 'close-range single-hand', 'free throw',
    '3-points challenge', 'mid-range challenge', 'layup'
]
...
class YoloTrtEngine:
    ...
    def __init__(self, engine_path: str, batch_size: int = 1, input_hw=(640, 640)):
...
def run_sequence(
    ...
    engine = YoloTrtEngine(engine_path, batch_size=1, input_hw=(640, 640))

    winner_evt = threading.Event()
    category_map = {0: "ball", 1: "bib", 2: "bob", 3: "player"}
    style = 'Left dribble challenge high'
    ...
    count = 20
    crop_display = [720, 40, 1200, 1100]
    feature_dribble = DribbleFeature(winner_evt, 1920, 1200, style, count, crop_display)
    ...
    for frame_idx, img_path in enumerate(img_paths):
        ...
        batch= chw.astype(engine.in_dtype, copy=False)
        batch = batch[None, ...]
